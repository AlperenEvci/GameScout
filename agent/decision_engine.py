...
from utils.helpers import get_loggers.helpers import get_logger
import randomdom
import timeme
import re
import sys
...
from data.map_data import get_nearby_points_of_interest, get_quests_for_regionoints_of_interest, get_quests_for_region
from data.web_search import search_game_content, get_region_informationfrom data.web_search import search_game_content, get_region_information
from llm.api_client import LLMAPIClient

# Add project root to Python path for importsimports
project_root = Path(__file__).parent.parent(__file__).parent.parent
if project_root not in sys.path:path:
    sys.path.append(str(project_root))    sys.path.append(str(project_root))

# Try to import BG3KnowledgeBase from query module# Try to import BG3KnowledgeBase from query module
try:
    from query import BG3KnowledgeBasey import BG3KnowledgeBase
except ImportError:
